...
app.put('/api/about', upload.single('image'), async (req, res) => {
  try {
    ...
    // Handle staff details - handle staff image upload
...
app.post('/api/about/staff', upload.single('image'), async (req, res) => {
  try {
    let about = await About.findOne();
    if (!about) {
...
// Delete staff
app.delete('/api/about/staff/:index', async (req, res) => {
  try {
